...
/**
 * List<Map<String,Object>>数据格式导出Excel工具类(多层标题合并单元格)
 * @author jinlong
 *
 */
public class ExportToExcelUtil {
	public void exportMain(HttpServletResponse response,String fileName,
			List<List<String>> head,List<List<String>> headnum,List<String> colName,List<Map<String, Object>>... dataList) throws Exception {
		int startRow = 1;
		...
		if(head != null && head.size() > 0) {
			if(headnum != null && headnum.size() > 0) {
				...
				for(int i = 0; i < head.size(); i++) {
					...
					for (int j = 0; j < head.get(i).size(); j++) {
			            ...
			            cell.setCellValue(head.get(i).get(j));
			            cell.setCellStyle(style);
			            sheet.autoSizeColumn((short)j); 
			        }
					//动态合并单元格
					if(i < headnum.size()) {
						for (int j = 0; j < headnum.get(i).size(); j++) {
				            temp = headnum.get(i).get(j).split(",");
				            startrow = Integer.parseInt(temp[0]);
				            overrow = Integer.parseInt(temp[1]);
				            startcol = Integer.parseInt(temp[2]);
				            overcol = Integer.parseInt(temp[3]);
			...
			for (int i = 0; i < dataList[0].size(); i++) {
	            ...
	            if(dataList[0].size()>1){
	                ...
	                for (int j = 0; j < colName.size(); j++) {
	                 	Map<String,Object> tempmap = dataList[0].get(i);
	                     if(tempmap!=null && tempmap.get(colName.get(j))!=null){
	                     	String data = tempmap.get(colName.get(j)).toString();                
