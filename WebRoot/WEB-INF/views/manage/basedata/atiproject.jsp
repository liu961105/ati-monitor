		...
		<table class="layui-table">
			...
			<script id="tradecent-template" type="text/x-handlebars-template">
                ...
                <tr>
					...
					<td><div class='base-wrap left' title="">{{createTime}}</div></td>
                    <td>
						<button type="button" class="layui-btn layui-btn-xs layui-btn-warm config-monitorSites" onclick = "ysjl('{{id}}')" data-toggle="modal"  data-pkid={{id}}>生成报告</button>
                    </td>
                </tr>
                {{/each}}
            </script>
		...
		<div class="layui-row">
			<div class="layui-col-md6">
				<div class="layui-form-item">
					<label class="layui-form-label">项目状态</label>
					<div class="layui-input-block">
						<select class="base-select validate require" name="projectStatus">
							<option value="0">正常</option>
							<option value="1">警告</option>
						</select>
					</div>
		...
		<div class="layui-row">
			...
			<div class="layui-col-md6">
				<div class="layui-form-item">
					...
					<div class="layui-input-block">
						<select class="base-select validate require projectStructure"
...
<script type="text/javascript">
	$(function() {
		$.baseEvent({
			ctx : '${ctx}',
			param : [ 'projectName', 'address', 'projectStatus', 'projectStructureId' ],
			isPage : true,
...
$(function(){
    ...
    $('#datalist').on('click', '.config-monitorSites', function(){
        ...
        var ID= $(this).data('pkid');
         location.href="${ctx}/dataReport/dataDownload?ID="+ID;
    })
    //保存方法
